...
const cell = cells[0];
cell.click();
if (!app.querySelector('.ev-edit')) fail('날짜를 눌러도 적는 칸이 안 열린다');
else ok('날짜를 누르면 적는 칸이 열린다');
...
// 그날 시간표로 가는 길은 날짜 숫자에 남는다 — 구글 캘린더도 그렇게 나눈다.
w.eval("state.eventEdit = null; state.view = 'month'; render();");
const first = [...app.querySelectorAll('.mo td')].find((td) => !td.classList.contains('out'));
first.querySelector('.d.go').click();
if (w.eval('state.view') !== 'day') fail('날짜 숫자를 눌러도 그날로 가지 않는다');
else ok('날짜 숫자를 누르면 그날 시간표로 간다');

console.log('\n[7-1] 「일」에서 수업을 누르면 그 교시로 적는다');
w.eval("state.eventEdit = null; state.view = 'day'; render();");
const lessonSlot = [...app.querySelectorAll('.day .slot:not(.meal)')].find((n) => n.textContent.includes('3교시'));
lessonSlot.querySelector('.cell').click();
const editor = app.querySelector('.ev-edit');
if (!editor) fail('수업을 눌러도 적는 칸이 안 열린다');
else ok('수업을 누르면 적는 칸이 열린다');
if (editor && editor.querySelector('select').value !== '3') fail(`교시가 안 들어갔다 — ${editor.querySelector('select').value}`);
else ok('3교시가 미리 들어간다');

console.log('\n[7-2] 「주」에서 수업을 눌러도 그 날짜·교시로 적는다');
w.eval("state.eventEdit = null; state.view = 'week'; render();");
const wkCell = [...app.querySelectorAll('.wk td.pick')][0];
wkCell.click();
const ed2 = app.querySelector('.ev-edit');
if (!ed2) fail('주간표에서 수업을 눌러도 안 열린다');
else ok('주간표에서도 열린다');
if (ed2 && !ed2.querySelector('select').value) fail('교시가 비어 있다');
else ok('교시가 미리 들어간다');
